...
/*   Updated: 2020/02/08 00:35:55 by ppetitea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_result	init_entity_listener(t_listener *self)
{
	...
	init_list_head(&self->hover_start_actions);
	init_list_head(&self->hover_end_actions);
...
t_result	init_entity_texture(t_entity_texture *self)
{
	...
	self->box.anchor = ft_vec2i(0, 0);
	self->box.scale = ft_vec2f(1.0f, 1.0f);
	self->box.offset = ft_vec2i(0, 0);
	self->box.size = ft_vec2i(1, 1);
	self->box.start = ft_vec2i(0, 0);
	self->box.end = ft_vec2i(1, 1);
	self->curr = NULL;
	self->curr_head = NULL;
	self->prev = NULL;
	self->prev_head = NULL;
