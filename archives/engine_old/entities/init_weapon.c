...
/*   Updated: 2020/01/25 02:18:27 by ppetitea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_weapon	*init_new_weapon()
{
	...
	init_list_head(&new->fire.first_person);
	init_list_head(&new->fire.third_person);
	init_list_head(&new->reload.first_person);
	init_list_head(&new->reload.third_person);
	new->type = HAND;
	new->name = NULL;
	new->ammo = 0;
	new->magazine_size = 0;
